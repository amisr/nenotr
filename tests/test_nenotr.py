...
exclude_nodes = ['/ProcessingParams/ProcessingTimeStamp','/ProcessingParams/SoftwareVersion',
                 '/Calibration/CalDate','/ProcessingParams/OutputPath']
...
class SingleFileNeNoTr(unittest.TestCase):
    def test_nenotr(self):

        # Run the nenotr program
        config = os.path.join(path,'config_test.ini')
        print(config)
        nenotr = CalcNeNoTr(config)
        nenotr.run()

        output_file = 'output/20171003.001_nenotr_20sec.h5'
        expected_file = 'expected/20171003.001_nenotr_20sec.h5'
